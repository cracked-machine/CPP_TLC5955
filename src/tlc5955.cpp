...
void Driver::set_function_cmd(DisplayFunction dsprpt, TimingFunction tmgrst, RefreshFunction rfresh, PwmFunction espwm,
                              ShortDetectFunction lsdvlt)
{
...
void Driver::set_dot_correction_cmd_all(uint8_t pwm)
{
    ...
    {
        noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, dc_pwm_cmd,
                                                   m_dc_data_offset + m_dc_data_size * dc_idx);
    }
}
...
void Driver::set_greyscale_cmd_rgb(uint16_t blue_pwm, uint16_t green_pwm, uint16_t red_pwm)
{
    ...
    {
        noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, blue_gs_pwm_cmd,
                                                   m_gs_data_offset + (m_gs_data_size * gs_idx * m_num_colour_chan));
        noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, green_gs_pwm_cmd,
                                                   m_gs_data_offset + (m_gs_data_size * gs_idx * m_num_colour_chan) +
                                                       m_gs_data_size);
        noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, red_gs_pwm_cmd,
                                                   m_gs_data_offset + (m_gs_data_size * gs_idx * m_num_colour_chan) +
                                                       (m_gs_data_size * 2));
    }
}
...
void Driver::set_greyscale_cmd_white(uint16_t pwm)
{
    ...
    {
        noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, gs_pwm_cmd,
                                                   m_gs_data_offset + m_gs_data_size * gs_idx);
    }
}

bool Driver::set_greyscale_cmd_rgb_at_position(uint16_t led_idx, uint16_t red_pwm, uint16_t green_pwm,
                                               uint16_t blue_pwm)
{
    ...
    noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, blue_gs_pwm_cmd,
                                               m_gs_data_offset + (m_gs_data_size * led_idx * m_num_colour_chan));
    noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, green_gs_pwm_cmd,
                                               m_gs_data_offset + (m_gs_data_size * led_idx * m_num_colour_chan) +
                                                   m_gs_data_size);
    noarch::bit_manip::insert_bitset_at_offset(m_common_bit_register, red_gs_pwm_cmd,
...
#pragma GCC diagnostic ignored "-Wvolatile"
    ...
    LL_GPIO_SetPinSpeed(m_serial_interface.get_mosi_port(), m_serial_interface.get_mosi_pin(),
                        LL_GPIO_SPEED_FREQ_VERY_HIGH);
    LL_GPIO_SetPinOutputType(m_serial_interface.get_mosi_port(), m_serial_interface.get_mosi_pin(),
    ...
    // Enable GPIO (SPI_SCK)
    LL_GPIO_SetPinSpeed(m_serial_interface.get_sck_port(), m_serial_interface.get_sck_pin(),
                        LL_GPIO_SPEED_FREQ_VERY_HIGH);
    LL_GPIO_SetPinOutputType(m_serial_interface.get_sck_port(), m_serial_interface.get_sck_pin(),
                             LL_GPIO_OUTPUT_PUSHPULL);
    ...
    m_serial_interface.get_gsclk_handle()->CCER =
        m_serial_interface.get_gsclk_handle()->CCER | m_serial_interface.get_gsclk_tim_ch();
